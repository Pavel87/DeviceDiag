...
@AndroidEntryPoint
class ThermalInfoActivity : ComponentActivity(), ExportTask.OnExportTaskFinished {

    private val viewModel by viewModels<ThermalViewModelKt>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val onBack = {
                interstitialAdManager.maybeShowInterstitial(this@ThermalInfoActivity) { finish() }
            }

            DeviceInfoTheme {
                Scaffold(topBar = {
                    TopBar(
                        title = stringResource(id = R.string.title_activity_thermal_info),
                        exportVisible = true,
        ...
        viewModel.observeThermalInfo(applicationContext)
    }
    ...
    private fun export() {
        if (!isExporting) {
            isExporting = true
            ExportTask(
                applicationContext, ThermalInfoKt.EXPORT_FILE_NAME, this
            ).execute(viewModel)
        }
    }
