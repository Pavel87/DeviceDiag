...
@AndroidEntryPoint
class BluetoothInfoActivity : ComponentActivity(), OnExportTaskFinished {

    private val viewModel: BluetoothViewModelKt by viewModels()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val onBack = {
                interstitialAdManager.maybeShowInterstitial(this@BluetoothInfoActivity) { finish() }
            }

            DeviceInfoTheme {
                Scaffold(topBar = {
                    TopBar(
                        title = stringResource(id = R.string.title_activity_bluetooth_info),
                        exportVisible = true,
                        onExportClick = {
                            export()
                        },
    ...
    private fun export() {
        if (!isExporting) {
            isExporting = true
            ExportTask(
                applicationContext, BluetoothInfoKt.EXPORT_FILE_NAME, this
            ).execute(viewModel)
        }
    }
