...
class BuildPropViewModelKt @Inject constructor() : ViewModel() {
    ...
    private val _filteredBuildProperties = MutableStateFlow(_buildProperties)
    val filteredBuildProperties: StateFlow<List<Pair<String, String?>>> = _filteredBuildProperties.asStateFlow()
    ...
    fun filterProperties(searchTerm: String) {
        _filteredBuildProperties.value = _buildProperties.filter { it.first.contains(searchTerm, ignoreCase = true) }
    }
